# WaveTips - Full Stack Streaming Alert & Donation Platform
...
7. Copy the MongoDB connection URI.
   *Example: `mongodb+srv://username:password@cluster0.mongodb.net/WaveTips`*
...
git init
git add .
git commit -m "Initial commit - WaveTips"

# Create a repo on GitHub, then:
git remote add origin https://github.com/YOUR_USERNAME/WaveTips.git
git branch -M main
...
```
https://WaveTips-api.onrender.com
```
...
| Variable | Value |
|----------|-------|
| `NEXT_PUBLIC_SERVER_URL` | `https://WaveTips-api.onrender.com` ← *your Render URL* |
...
```
https://WaveTips.vercel.app
```

> [!IMPORTANT]
> After getting your Vercel URL, go back to **Render** and update the `CORS_ORIGIN` environment variable to match your Vercel URL (e.g., `https://WaveTips.vercel.app`). Then redeploy the backend.
...
```
CORS_ORIGIN=https://WaveTips.vercel.app
```
...
3. Add your backend URL:
   ```
   https://WaveTips-api.onrender.com/api/tips/verify-payment
   ```

### 4.4 — Custom Domain (Optional)
- **Vercel**: Settings → Domains → Add your domain (e.g., `WaveTips.com`)
- **Render**: Settings → Custom Domains → Add your API domain (e.g., `api.WaveTips.com`)
...
```bash
# Test backend health
curl https://WaveTips-api.onrender.com/api/health
...
sequenceDiagram
    ...
    Render-->>You: https://WaveTips-api.onrender.com
    You->>Vercel: Connect repo, set root=client
    Vercel-->>You: https://WaveTips.vercel.app
...
This guide walks you through deploying **WaveTips**, specifically detailing how to set up the database and authentication layers assuming you are starting completely fresh.
...
2. Click **New Project**, select an organization, and give your project a name (e.g., *WaveTips*).
...
2. Import the GitHub repository for WaveTips.
...
2. **Restrict CORS**: Go to the **Render Dashboard**, edit the WaveTips-backend environment variables, and change CORS_ORIGIN from * to https://<YOUR-VERCEL-FRONTEND-URL>.
